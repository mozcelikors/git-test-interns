...
int thread_function(void *pv)
{
    ...
    while(!kthread_should_stop()) {
        pr_info("Thread Counter %ld\n", i++);
        snprintf(string, 100, "%ld\n", i);
        strcpy(kernel_buffer, string);
        msleep(1000);
    }
    return 0;
}
                ...
                const char __user *buf, size_t len, loff_t *off)
{
        ...
        int k = kstrtol(kernel_buffer,10,&i);
        if (k==0){

                printk("kstrol successed\n");
        }
...
static int __init etx_driver_init(void)
{
        ...
        /*Creating device*/
        if((device_create(dev_class,NULL,dev,NULL,"sahin_device")) == NULL){
